...
def compute_pose_score(
    ...
    # This function's return value is a similarity (0-1, higher = better match),
    # deliberately inverted from the raw keypoint distance so every metric in
    # this file shares the same "higher is better" convention (matching the
    # ArcFace identity score). POSE_THRESHOLD in config.py is still expressed
    # as a distance-style value (lower = stricter) and is converted at the
    # call site: passes["pose"] = pose >= (1.0 - POSE_THRESHOLD).
...
def compute_beat_alignment_ms(
    ...
    Important scope note: this compares the shot's storyboard-level
    beat_time_s (set in stage1/stage2, before generation) against the
    nearest detected reference beat. It evaluates the cut-to-beat decision
    made at the template stage, not the timing of the final assembled
    output video. On the executed dissertation runs, each Kling I2V clip
    is generated at a fixed 5s regardless of the shot's beat-aligned
    duration, so beat timing is not propagated to the final assembled
    video (dissertation §4.5 / §4.15.1). A "pass" here says the storyboard
    cut point was well-aligned to the reference beat, not that the final
    video is beat-accurate.
    """
