...
| `beach/decision_log.json` | Per-shot generation record | Summarises per-shot attempt counts, review status, generation settings and approval notes for each keyframe and clip |
...
| `street/decision_log.json` | Per-shot generation record | Same structure as the beach log, plus the logged human-guided motion-repair event for shot_001 and shot_003 |
