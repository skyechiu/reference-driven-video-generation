...
\textbf{Input:} unit $u_i$, identity package $I$, look $L$, scene $S$, max attempts $N_{\text{max}}$\\
\textbf{Output:} approved keyframe $k_i$, clip $c_i$, or \texttt{needs\_human}\\[2pt]
1: slots $\leftarrow$ OrderReferenceSlots($I,L,S,u_i$.source\_frame) \quad\{slot[0] = primary identity\}\\
2: positive\_prompt $\leftarrow$ BuildPromptBlocks(CHARACTER, LOOK, SCENE, FRAMING, ACTION)\\
3: negative\_record $\leftarrow$ BuildNegativeRecord($u_i$) \quad\{stored separately; never passed to the API, \S{}3.5.2\}\\
4: attempt $\leftarrow 0$; verdict $\leftarrow$ reject\\
5: \textbf{while} verdict $\neq$ approve \textbf{and} attempt $<$ $N_{\text{max}}$ \textbf{do}\\
6: \quad attempt $\leftarrow$ attempt $+1$\\
7: \quad $k \leftarrow$ ImagesEdit(slots, positive\_prompt) \quad\{\texttt{gpt-image-1}; forbidden-word guard checks positive\_prompt against negative\_record first\}\\
8: \quad verdict $\leftarrow$ HumanReview($k$)\\
9: \quad \textbf{if} verdict $=$ reject \textbf{then}\\
10: \quad\quad issue $\leftarrow$ Diagnose($k,u_i$)\\
11: \quad\quad positive\_prompt $\leftarrow$ Revise(positive\_prompt, issue) \quad\{targeted, not blind re-roll\}\\
12: \quad \textbf{else}\\
13: \quad\quad issue $\leftarrow$ none\\
14: \quad \textbf{end if}\\
15: \quad LogAttempt($u_i$.shot\_id, attempt, issue, positive\_prompt, verdict) \quad\{every attempt logged\}\\
16: \textbf{end while}\\
17: \textbf{if} verdict $\neq$ approve \textbf{then}\\
18: \quad mark $u_i$.shot\_id \texttt{needs\_human}; \textbf{return}\\
19: \textbf{end if}\\
20: $c \leftarrow$ KlingI2V($k, u_i$.character\_motion) \quad\{prompt-level I2V\}\\
21: \textbf{return} $k, c$
\end{algobox}
...
\textbf{Condition A --- background-preserving replacement.} Given a \textbf{driving performance video}, the system preserves the original background, camera and timing and replaces only the performer. Two per-frame conditioning signals are extracted from the driving video: a \textbf{DWPose skeleton} and a \textbf{SAM-style person mask} (Segment Anything family; Kirillov et al., 2023) --- see Figure 3.9. These condition Wan2.2-Animate (Replacement Mode) so the target character follows the driving motion while the background \emph{outside} the mask is preserved. Unlike Mode A's prompt-level I2V, the pose here is a genuine per-frame structural control consumed by the backend.
...
The build order within the milestones follows the same risk-first logic: Phase 0, then the analysis and template stages, then per-shot generation, then the automatic evaluation and repair loop, with interface and packaging deliberately last. Figure 3.10 shows the \emph{planned} schedule as a timeline. The plan is notable for placing the highest-risk task (Phase 0) second, immediately after scoping, so that the risk to the whole approach would be assessed early in the build; in execution that test was not completed before the reference run, and the pre-committed keyframe-first fallback was adopted directly (\S{}3.5.4) rather than the risk being resolved as planned.

\begin{figure*}[!tp]
\centering
\includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{figs/project_timeline.png}
\par\vspace{3pt}{\footnotesize \textbf{Figure 3.10} --- Planned project timeline (2026). The build follows a risk-first order: the highest-risk component, the Phase 0 pose-vs-identity feasibility test, was scheduled immediately after scoping so that the risk to the whole approach would be assessed early. In execution this test was not completed before the reference run, and the pre-committed keyframe-first fallback was adopted directly (\S{}3.5.4, \S{}3.11); the analysis, generation and evaluation--repair stages follow as planned, with the scene-replacement and Mode C extensions and the write-up last.}
\end{figure*}
...
\textbf{What is verified now.} The system produces a complete, coherent short-form video from a reference video plus a fixed IP character and a new scene, end to end, on a real four-shot run. Cross-shot \textbf{identity consistency, look fidelity and framing} were confirmed by \textbf{human visual review}, and the generation stage repairs its own failures through a logged, human-gated diagnose-and-retry loop (Table 4.5): three of the four shots required at least one corrective regeneration and all four ultimately passed review. This is a genuine result --- a working reference-driven, IP-conditioned, human-gated, selectively-repairable generation pipeline --- and it is defensible on its own terms.
