...
Run this AFTER scripts/generation/ablation_blind_reroll_generate.py --confirm has produced files under
    ...
    for sid in ["shot_001", "shot_003"]:
        ...
        if not blind_attempts:
            rows.append({
                ...
                "recovered_motion_pct": "", "accepted": "PENDING — run scripts/generation/ablation_blind_reroll_generate.py --confirm",
                "human_verdict": "",
            })
        else:
...
"""
    ...
    if gen_log is None:
        print("\nNOTE: generation_log.json not found — this ran on Condition B only. "
              "Run scripts/generation/ablation_blind_reroll_generate.py --confirm first for Condition A data.")
