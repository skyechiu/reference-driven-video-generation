...
summary = json.loads((OUT / "export_summary.json").read_text())

readme = f"""# Reference-Driven Agentic Short-Form Video Generation System — static evidence site

This folder is a fully static export of the read-only dashboard (`pipeline/app.py`),
generated on {summary['generated_at_utc']} for deployment to a Hugging Face Static Space
...
"""
(OUT / "README.md").write_text(YAML_FRONTMATTER + readme, encoding="utf-8")

# ---- zip with index.html at zip root -------------------------------------
zip_path = ROOT / "outputs" / "huggingface_static_site.zip"
# NOTE: do NOT zip_path.unlink() an existing file here — the project folder
# is a FUSE mount that rejects unlink() ("Operation not permitted") even
# though it happily allows truncate-write via open("w")/ZipFile's "w" mode.
# Opening in "w" mode below truncates-in-place, which works fine.
...
print("README written, zip built at", zip_path)
